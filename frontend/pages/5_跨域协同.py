"""
frontend.pages.5_跨域协同 - A2A 跨域 Agent 协同演示
...
st.set_page_config(page_title="跨域协同", page_icon="🤝", layout="wide")
st.title("🤝 跨域协同（A2A）")
...
if "不动产" in agent_type:
    ...
    if st.button("🔍 查询不动产", type="primary", use_container_width=True):
    ...
    if st.button("🔍 查询公积金", type="primary", use_container_width=True):
